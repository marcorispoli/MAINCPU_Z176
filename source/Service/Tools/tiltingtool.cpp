...
// _PG_SERVICE_TOOLS_TILT  codice pagina
// <TAG6>  codice pagina exit


#include "tiltingtool.h"
#include "ui_tilting.h"
...
#define UI_PAGINA _PG_SERVICE_TOOLS_TILT
#define EXIT_PAGINA _PG_SERVICE_TOOLS_MENU
#define EXIT_BUTTON ui->exitButton
#define DISABLE_EXIT_TMO    1000

tiltingtool::tiltingtool(int rotview, QWidget *parent) :
    QWidget(parent),
    ui(new Ui::tiltingToolUI)
{
    ui->setupUi(this);
...
void tiltingtool::changePage(int pg, int opt)
{
    if(UI_PAGINA==pg)
    {
...
void tiltingtool::onExitButton(void)
{

    GWindowRoot.setNewPage(EXIT_PAGINA,GWindowRoot.curPage,0);
}

void tiltingtool::initPage(void){
...
void tiltingtool::exitPage(void){
...
void tiltingtool::timerEvent(QTimerEvent* ev)
{
    ...
    if(ev->timerId()==timerRequest)
    {
        ...
        pConfig->pSlaveMcc->sendFrame(MCC_GET_TRX_INPUTS,1,data,1); // attiva la modalità di calibrazione potenziometro

    }
}

// FUNZIONE DI AGGIORNAMENTO CAMPI VALORE CONNESSO AI CAMPI DEL DATABASE
void tiltingtool::valueChanged(int index,int opt)
{
    switch(index){
    case _DB_SERVICE1_INT:
        if(ApplicationDatabase.getDataI(index)) ui->faultFrame->show();
        else ui->faultFrame->hide();
        break;
    case _DB_SERVICE2_INT:
        if(ApplicationDatabase.getDataI(index)) ui->obstcleFrame->show();
        else ui->obstcleFrame->hide();
        break;
    case _DB_SERVICE3_INT:
        if(ApplicationDatabase.getDataI(index)) ui->expWinFrame->show();
        else ui->expWinFrame->hide();
        break;
    case _DB_SERVICE4_INT:
        if(ApplicationDatabase.getDataI(index)) ui->zeroFrame->show();
        else ui->zeroFrame->hide();
        break;
    ...
    case _DB_SERVICE6_INT: // Hardware configuration
...
void tiltingtool::slaveNotifySlot(unsigned char id,unsigned char mcc_code,QByteArray data)
{
    unsigned long val;

    if(mcc_code==MCC_GET_TRX_INPUTS){
        // Dati relativi al potenziometro
        //val = data[0] + data[1]*256 + data[2]*256*256 + data[3]*256*256*256;

        // Verifica Inputs:
        if(data[0]&1) ApplicationDatabase.setData(_DB_SERVICE3_INT,(int) 1);
        ...
        if(data[0]&4) ApplicationDatabase.setData(_DB_SERVICE4_INT,(int) 1);
        else ApplicationDatabase.setData(_DB_SERVICE4_INT,(int) 0);

        if(data[0]&8) ApplicationDatabase.setData(_DB_SERVICE1_INT,(int) 1);
