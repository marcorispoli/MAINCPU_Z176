...
void serverDebug::PCB244A_Notify(unsigned char id, unsigned char mcccode, QByteArray buffer)
{
    ...
    case MCC_PCB244_A_GET_ID:
     ...
     // Feedback comando di attivazione/Disattivazione modalit‡ manuale
...
void serverDebug::handleCompressore(QByteArray data)
{
    ...
    }else if(data.contains("calibThresholds"))
    {
        ...
        serviceTcp->txData(QByteArray("I dati devono essere salvati e riletti perch√® siano operativi \r\n"));
...
void serverDebug::handleSetThick(QByteArray data)
{
    ...
    if(pCompressore->isValidPad()==FALSE)
    {
        serviceTcp->txData(QByteArray("Questa funzione pu√≤ essere eseguita solo con un Pad configurato!\n\r"));
        return;
    }
 ...
 *          - starter HS/LS/OFF                  Attiva Starter ad Alta/Bassa velocit√  o OFF
 */
